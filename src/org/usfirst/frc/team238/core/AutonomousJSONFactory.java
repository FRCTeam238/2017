...
public class AutonomousJSONFactory implements AutonomousState {
   ...
   * 
   * @param AutoModesArrayList
   *          (Gets created in the autonomousController, and is sent here from
   *          the robot)
   * @param AutoModeNames
   *          (Gets created in the autonomousController, and is sent here from
   *          the robot)
  ...
  /**
   * Builds a new JSON file using the AutoModeArrayList and the AutoModeNames
  ...
  public String BuildNewJson(ArrayList<AutonomousState>[] AutoModesArrayList, ArrayList<String> AutoModeNames) {
    ...
    try {
      ...
      while (ModeIterator.hasNext()) {
        ...
        // Make the CommandArrayList equal to a certain mode in an
        // autoModeArrayList
        ...
        while (CommandIterator.hasNext()) {
          ...
          JSONObject jsonCommandObject = new JSONObject(); // Object that holds
                                                           // the name and
                                                           // params
          // Create a new string for this specific CommandName
          commandName = CurrentCommandState.toString();
          // Make a substring for this CommandName so that it has just the name,
          ...
          jsonAutoModeArrayObject.put("Commands", jsonCommands); // Add a
                                                                 // Command
                                                                 // Array to the
                                                                 // mode
          ...
          // Increments through all the parameters in this command (If there's
          // any)
          for (int i = 0; i < 4; i++) {
